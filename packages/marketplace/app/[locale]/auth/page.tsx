...
function AuthError({ message, className }: { message: string; className?: string }) {
  if (!message) return null;
  return (
    <div className={`p-4 bg-aurora-red/10 border border-aurora-red/20 rounded-lg${className ? ` ${className}` : ''}`}>
      <p className="text-sm text-aurora-red">{message}</p>
    </div>
  );
}

function EmailPill({ email, label, onClick }: { email: string; label: string; onClick: () => void }) {
  return (
    <div className="flex justify-center mb-6">
      ...
      >
        ...
        <span className="text-xs text-frost-ice font-semibold ml-1">{label}</span>
      </button>
    </div>
  );
}
...
export default function AuthPage() {
  ...
  const tComingSoon = useTranslations('Auth.comingSoon');
  ...
  // Get redirect and error parameters from URL on mount
  useEffect(() => {
    ...
    // Display error from /auth/confirm redirect (e.g., expired magic link)
  ...
  const handleEmailSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // Show error and return to email entry instead of silently defaulting
      setError(tErrors('checkFailed'));
    ...
    try {
      ...
      if (authState === 'new_user' && turnstileRef.current) {
        try {
          ...
          // If Turnstile times out or errors, allow through but log for monitoring
          console.warn('Turnstile verification skipped:', turnstileError instanceof Error ? turnstileError.message : 'Unknown error');
        }
      }
  ...
  const renderEmailEntry = () => (
    <>
      ...
      <Card padding="lg">
        ...
        {/* Divider + OAuth Buttons — hidden in coming-soon mode to force email-first flow */}
        {!isComingSoon && (
          <>
  ...
  // Render registrations closed (Coming Soon mode — new user blocked)
  const renderRegistrationsClosed = () => (
    <>
      <div className="text-center mb-8">
        <div className="w-16 h-16 rounded-2xl bg-frost-ice/10 flex items-center justify-center mx-auto mb-4 border border-frost-ice/10">
          <svg
  ...
  const renderNewUser = () => (
    <>
      ...
      <Card padding="lg">
        <AuthError message={error} className="mb-5" />

        <EmailPill email={email} label={tNewUser('changeEmail')} onClick={handleChangeEmail} />
        ...
        <p className="text-xs text-text-muted text-center mt-5">
          ...
          <Link href="/legal/terms" className="text-frost-ice hover:underline" target="_blank">
            {tNewUser('termsOfService')}
          </Link>{' '}
          {tNewUser('termsAnd')}{' '}
          <Link href="/legal/privacy" className="text-frost-ice hover:underline" target="_blank">
    ...
    return (
      <>
        ...
        <Card padding="lg">
          ...
          <EmailPill email={email} label={tExistingUser('changeEmail')} onClick={handleChangeEmail} />
  ...
  const renderContent = () => {
    switch (authState) {
      ...
      case 'new_user':
        return isComingSoon ? renderRegistrationsClosed() : renderNewUser();
      case 'existing_user':
  ...
  // Show loading spinner while checking auth state (prevents flash for logged-in users)
  if (authLoading) {
    return (
