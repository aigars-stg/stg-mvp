...
import { Button, Card, ListingThumbnail } from '@second-turn/design-system';
import { Star, ArrowLeft, RefreshCw as Loader2, CheckCircleAlt01 as CheckCircle2 } from '@/lib/icons';
    ...
    <div className="min-h-screen bg-bg-secondary py-6 px-4 sm:px-6">
      <div className="max-w-lg mx-auto">
        ...
        <Card className="p-6">
          ...
          {/* Order Summary */}
          {order && (
            <div className="flex items-center gap-3 bg-bg-secondary rounded-xl p-3 mb-6">
              <ListingThumbnail
                src={order.items[0]?.photo_url}
                alt={order.items[0]?.game_name ?? 'Game'}
                size="md"
              />
              <div className="min-w-0">
                <p className="font-medium text-polar-night truncate">
                  ...
                  {order.items.length > 1 && (
                    <span className="text-text-muted font-normal"> +{order.items.length - 1} more</span>
                  )}
                </p>
                <p className="text-sm text-text-secondary">
                  {order.order_number} · Sold by {order.seller_name}
                </p>
              </div>
            </div>
          )}
